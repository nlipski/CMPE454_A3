...

{
  ...
  out.rows[0][0] =  (rows[1][1]*rows[2][2]-rows[2][1]*rows[1][2])*invdet;
  out.rows[1][0] = -(rows[0][1]*rows[2][2]-rows[0][2]*rows[2][1])*invdet;
  out.rows[2][0] =  (rows[0][1]*rows[1][2]-rows[0][2]*rows[1][1])*invdet;
  out.rows[0][1] = -(rows[1][0]*rows[2][2]-rows[1][2]*rows[2][0])*invdet;
  out.rows[1][1] =  (rows[0][0]*rows[2][2]-rows[0][2]*rows[2][0])*invdet;
  out.rows[2][1] = -(rows[0][0]*rows[1][2]-rows[1][0]*rows[0][2])*invdet;
  out.rows[0][2] =  (rows[1][0]*rows[2][1]-rows[2][0]*rows[1][1])*invdet;
  out.rows[1][2] = -(rows[0][0]*rows[2][1]-rows[2][0]*rows[0][1])*invdet;
  out.rows[2][2] =  (rows[0][0]*rows[1][1]-rows[1][0]*rows[0][1])*invdet;
...

{
  ...
  vec3 F = (centre - eye).normalize();
  up = up.normalize();
  vec3 s = F ^ up;
  vec3 u = s.normalize() ^ F;

  M.rows[0] = vec4(  s[0],  s[1],  s[2], 0 );
  M.rows[1] = vec4(  u[0],  u[1],  u[2], 0 );
  M.rows[2] = vec4( -F[0], -F[1], -F[2], 0 );
  M.rows[3] = vec4(     0,     0,     0, 1 );

  return M * translate( -1 * eye ); 
}
