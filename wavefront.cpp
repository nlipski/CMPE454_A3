...
void wfModel::read( const char *basename, const char *filename )

{
  ...
  char pathname[ strlen(basename) + strlen(filename) + 1 ];
  sprintf( pathname, "%s%s", basename, filename );
  ...
  while(fscanf(file, "%s", buf) != EOF) {
	...
	readMaterialLibrary( basename, buf );
	break;
...
void wfModel::readMaterialLibrary( const char *basename, char *name )

{
  ...
  char pathname[ strlen(basename) + strlen(name) + 1 ];
  sprintf( pathname, "%s%s", basename, name );

  /* open the file */

  file = fopen(pathname, "r");
  if (!file) {
    cerr << "wfModel::readMaterialLibrary() couldn't open file '" << pathname << "'" << endl;
    exit(-1);
  }
