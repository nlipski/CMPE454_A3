...
class wfModel {
  ...
  void        readMaterialLibrary( const char *basename, char *filename ); /* read all materials */
 ...
 public:
  ...
  wfModel() {
    ...
    objToWorldTransform = identity();
  }

  wfModel( const char *basename, const char *filename ) {
    texturesInitialized = false;
    pathname = mtllibname = NULL;
    objToWorldTransform = identity();
    read( basename, filename );
    setupVAO();
  }

  ~wfModel() {}

  void read( const char *basename, const char *filename );         /* instantiate this model from a file */
